        ...
        <Router>
            <Layout lang={lang} setLang={setLang} t={t} theme={theme} toggleTheme={toggleTheme}>
                <Routes>
                    ...
                    <Route path="/notes/all" element={<AllNotes lang={lang} t={t} />} />
            ...
            <nav className="fixed top-0 w-full z-[100] glass border-b border-white/5">
                <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                    <Link to="/#hero" className="text-xl font-bold tracking-tighter font-mono cursor-pointer">
                        <span className="text-ue-blue">SU</span> <span className={theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}>P.C.</span>
                    </Link>

                    <div className="hidden lg:flex space-x-8 text-sm font-medium items-center font-sans">
                        <div className="relative group px-2">
                            <Link to="/#hero" className={`hover:text-ue-blue transition-colors flex items-center gap-2 py-1 px-4 ${location.pathname === '/' ? 'text-ue-blue' : (theme === 'dark' ? '' : 'text-slate-700')}`}>
                                {t('nav-about')}
                            ...
                            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 min-w-[140px] w-full invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 transform origin-top -translate-y-1 group-hover:translate-y-0 z-[110]">
                                <div className="glass rounded-xl border border-white/10 overflow-hidden shadow-xl text-center bg-white/5 backdrop-blur-md">
                                    <div className="py-1">
                                        <Link to="/#skills" className={`block px-4 py-3 text-sm hover:text-ue-blue hover:bg-white/5 transition-colors whitespace-nowrap ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>
                                            {t('nav-skills')}
                                        </Link>
                                        <Link to="/#experience" className={`block px-4 py-3 text-sm hover:text-ue-blue hover:bg-white/5 transition-colors whitespace-nowrap ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>
                                            {t('nav-exp')}
                                        </Link>
                                        <Link to="/#projects" className={`block px-4 py-3 text-sm hover:text-ue-blue hover:bg-white/5 transition-colors whitespace-nowrap ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>
                                            {t('nav-projects')}
                                        </Link>
                                        <Link to="/#contact" className={`block px-4 py-3 text-sm hover:text-ue-blue hover:bg-white/5 transition-colors whitespace-nowrap ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>
                                            {t('nav-contact')}
                                        </Link>
                                    </div>
                                </div>
                        ...
                        <Link to="/notes" className={`hover:text-ue-blue transition-colors ${location.pathname === '/notes' ? 'text-ue-blue' : (theme === 'dark' ? '' : 'text-slate-700')}`}>
                            {t('nav-notes')}
                        </Link>

                        <div className="flex items-center space-x-1 ml-4 bg-slate-900/50 p-1 rounded-lg border border-white/5">
                            <button onClick={() => setLang('en')}
                                className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${lang === 'en' ? 'bg-ue-blue text-white' : 'text-slate-400 hover:text-white'}`}>EN</button>
                            <button onClick={() => setLang('zh')}
                                className={`px-2 py-1 rounded text-[10px] font-bold transition-all ${lang === 'zh' ? 'bg-ue-blue text-white' : 'text-slate-400 hover:text-white'}`}>中文</button>
                        </div>

                        {/* Theme Toggle Button */}
                        <button
                            onClick={toggleTheme}
                            className="ml-4 p-2 rounded-lg bg-slate-900/50 hover:bg-slate-800 transition-all border border-white/5 group"
                    ...
                    <div className="lg:hidden flex items-center space-x-4">
                        ...
                        <div className="flex items-center space-x-1 bg-slate-900/50 p-1 rounded-lg border border-white/5">
                            <button onClick={() => setLang('en')}
                                className={`px-2 py-1 rounded text-[10px] font-bold ${lang === 'en' ? 'bg-ue-blue text-white' : 'text-slate-400'}`}>EN</button>
                            <button onClick={() => setLang('zh')}
                                className={`px-2 py-1 rounded text-[10px] font-bold ${lang === 'zh' ? 'bg-ue-blue text-white' : 'text-slate-400'}`}>ZH</button>
                        </div>
                    </div>
                </div>
            </nav>
