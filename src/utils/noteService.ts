...
export const noteService = {
    getNotes: (): Note[] => {
        ...
        // 1. Get static notes and filter out those that were explicitly deleted
        const staticNotes = (notesData as Note[]).filter(n => !deletedIds.includes(n.id));

        // 2. Build final list using a Map to let customNotes (edits) override static ones with same ID
        const notesMap = new Map<number, Note>();

        staticNotes.forEach(n => notesMap.set(n.id, n));
        customNotes.forEach(n => notesMap.set(n.id, n)); // Custom notes/edits take precedence
